...
cp_parser_class_specifier_1 (cp_parser* parser)
{
  ...
  if (flag_vtable_verify && bases)
    {
      tree derived_class_id = NULL_TREE;
      if (TREE_CHAIN (type)
          && TREE_CODE (TREE_CHAIN (type)) == TYPE_DECL)
        derived_class_id = get_mangled_id (TREE_CHAIN (type));
	 ...
	 verification).  */
      if (TREE_CODE (bases) == TREE_LIST)
        {
          tree cur_node;
          for (cur_node = bases; cur_node;
               cur_node = TREE_CHAIN (cur_node))
            {
              tree tree_val   = TREE_VALUE (cur_node);
              if ((TREE_CODE (tree_val) == RECORD_TYPE)
                  && TREE_CHAIN (tree_val)
                  && TREE_CODE (TREE_CHAIN (tree_val)) == TYPE_DECL)
                {
                  tree base_decl;
                  tree base_id;
                  tree var_id;
