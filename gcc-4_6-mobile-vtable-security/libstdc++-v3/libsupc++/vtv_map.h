...
// basic hash table with open addressing.  The keys are not "owned" by the
// table; it only stores pointers to keys.  The key type is specified below
// (see insert_only_hash_map::key_type) and is, roughly speaking, a string
// of any length with the string length and a hash code stored at the front.
// The code here does not compute any hash codes, but rather uses what's given.
...
class insert_only_hash_map {
 public:
  ...
  // Keys are a byte string (up to 2^32 - 1 long) plus a uint32_t that's used
  // as a hash code.  The latter can encode arbitrary information at the
  // client's discretion, so, e.g., multiple keys that are the same string still
  // "differ" if the hash codes differ.  Keys are equal if the first 8 bytes are
  // equal and the next n bytes are equal.
  struct key_type {
    uint32_t n;
    uint32_t hash;
    char bytes[0];
  ...
  // Create an empty map with a reasonable number of buckets for the expected
  // size.  Returns NULL if the allocator fails.
  ...
  // If *k is a key in this then set *v to point to the corresponding value.
  // Otherwise, do the equivalent of insert(k, value_type()) and, if that
  // succeeds, set *v to point to the inserted value.  Requires k to be
  // non-NULL.  Does nothing and returns NULL if the allocator fails.
  // Typically returns this, but will return a new insert_only_hash_map if a
  // resize occurs.  If the return value is non-NULL, *v is set and it's
  // valid until a resize of the map that is the return value.
  insert_only_hash_map*
  find_or_add_key(const key_type* k, value_type** v);

  // Get the value corresponding to *k.  Returns NULL if there is none.
  // Requires k to be non-NULL.  The return value is valid until any resize.
  const value_type*
 ...
 private:
  ...
  insert_only_hash_map*
  put_internal(const key_type*, const value_type&, bool);
  ...
  // Return a copy with double the number of buckets.
  // Returns NULL if the allocator fails.  Otherwise, calls
  // destroy(this).
  insert_only_hash_map*
  destructive_copy();

  size_type num_buckets_;  // Must be a power of 2 not less than min_capacity.
...
insert_only_hash_map<T, Alloc>::destructive_copy()
{
  ...
  for (size_type i = 0; i < this->bucket_count(); i++)
    if (this->buckets[i].first != NULL)
      copy->put_internal(this->buckets[i].first, this->buckets[i].second, true);
      ...
      if (bucket.first == NULL)
        {
          ...
          if (this->is_too_full(this->size() + 1))
            {
              insert_only_hash_map<T, Alloc>* result = this->destructive_copy();
              return result == NULL
                  ? NULL
  ...
  for (;;)
    {
      ...
      else if (!unique_key_and_resize_not_needed
               && bucket.first->equals(k))
        {
...
insert_only_hash_map<T, Alloc>::get(
    const insert_only_hash_map::key_type* k) const
{
  // Table size is always a power of 2.
  const size_type mask = this->bucket_count() - 1;
  size_type bucket_index = k->hash & mask;
