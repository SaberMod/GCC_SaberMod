...
set_bbs_move_to ()
{
  ...
  availloc = sbitmap_alloc (last_basic_block);
  trans = sbitmap_alloc (last_basic_block);
  availin = sbitmap_vector_alloc (last_basic_block, last_basic_block);
  availout = sbitmap_vector_alloc (last_basic_block, last_basic_block);
  bitmap_clear (trans);
  bitmap_vector_clear (availin, last_basic_block);
  bitmap_vector_clear (availout, last_basic_block);
  worklist.create (0);
...
/* Try to promote fp settings from the bbs in the dominator tree
   ...
   will be set to true.  */
void
evaluate_promote_fpset_to_bb (
		struct dom_walk_data *walk_data ATTRIBUTE_UNUSED,
...
choose_places (vec<rtx> *global_inserts)
{
  ...
  basic_block prologue_bb = single_succ (ENTRY_BLOCK_PTR);
  ...
  walk_data.before_dom_children = NULL; 
  walk_data.after_dom_children = evaluate_promote_fpset_to_bb;
...
move_fp_settings ()
{
  ...
  calls.create(0);
  bb_fpsets = XNEWVEC (struct bb_fpset, last_basic_block);
  memset (&bb_fpsets[0], 0,
	  sizeof (struct bb_fpset) * last_basic_block);
