...
public:
  ...
  static int eq_node_assembler_name (const void *p1, const void *p2);
...
cgraph_edge::recursive_p (void)
{
  ...
  cgraph_node *rcaller = caller;
  if (caller->global.inlined_to)
   rcaller = caller->global.inlined_to;

  if (L_IPO_COMP_MODE && cgraph_pre_profiling_inlining_done)
    {
      c = cgraph_lipo_get_resolved_node (c->decl);
      rcaller = cgraph_lipo_get_resolved_node (rcaller->decl);
    }
  return rcaller->decl == c->decl;
}
