...
symtab_node::unregister (void)
{
  ...
  gcc_assert (decl->decl_with_vis.symtab_node || in_lto_p
							/* LIPO may clear up the builtin's symtab node during 
								 pop scope */
							|| L_IPO_COMP_MODE);
...
symtab_node::verify_symtab_nodes (void)
{
  ...
  FOR_EACH_SYMBOL (node)
      ...
      if (node->get_comdat_group ())
	{
	  ...
	  if (!existed)
	    *entry = node;
            // relax the check for LIPO -- nodes can exist in different module groups.
	  else if (!DECL_EXTERNAL (node->decl) && !L_IPO_COMP_MODE)
	    {
