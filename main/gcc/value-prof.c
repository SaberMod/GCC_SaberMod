...
static bool
gimple_ic_transform_single_targ (gcall *call_stmt, histogram_value histogram)
{
  ...
  bb_all = gimple_bb (call_stmt)->count;
  /* The order of CHECK_COUNTER calls is important -
     since check_counter can correct the third parameter
     and we want to make count <= all <= bb_all. */
  if ( check_counter (call_stmt, "ic", &all, &bb_all, bb_all)
      || check_counter (call_stmt, "ic", &count, &all, all))
    {
      gimple_remove_histogram_value (cfun, call_stmt, histogram);
      return false;
    }
	  ...
	  if (dump_file)
	    {
	      ...
	      print_generic_expr (dump_file, gimple_call_fn (call_stmt), TDF_SLIM);
  ...
  if (!check_ic_target (call_stmt, direct_call))
    {
      if (dump_file)
	{
...
static bool
gimple_ic_transform_mult_targ (gcall *stmt, histogram_value histogram)
{
...
gimple_ic_transform (gimple_stmt_iterator *gsi)
{
  ...
  if (gimple_call_fndecl (call_stmt) != NULL_TREE)
    return false;

  if (gimple_call_internal_p (call_stmt))
    return false;

  histogram = gimple_histogram_value_of_type (cfun, call_stmt, HIST_TYPE_INDIR_CALL);
  if (!histogram)
    {
      histogram = gimple_histogram_value_of_type (cfun, call_stmt,
  ...
  if (histogram->type == HIST_TYPE_INDIR_CALL)
    return gimple_ic_transform_single_targ (call_stmt, histogram);
  else
    return gimple_ic_transform_mult_targ (call_stmt, histogram);
}
