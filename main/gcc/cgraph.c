...
cgraph_edge::remove_caller (void)
{
  ...
  if (caller->call_site_hash && call_stmt)
...
{
  if (cgraph_node::get (decl)
      && cgraph_is_aux_decl_external (cgraph_node::get (decl)))
...
cgraph_node::dump (FILE *f)
{
  ...
  if (count)
    fprintf (f, " executed %"PRId64"x",
	     (int64_t)count);
  if (max_bb_count)
    fprintf (f, " hottest bb executed %"PRId64"x",
	     (int64_t)max_bb_count);
  if (origin)
