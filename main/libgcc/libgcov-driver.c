...
static void
write_one_data (struct gcov_info *gi_ptr,
...
static void
gcov_dump_module_info (struct gcov_info *list, struct gcov_filename *gf)
{
  struct gcov_info *gi_ptr;
	...
	continue;
      ...
      if ((error = gcov_close ()))
        gcov_error (error  < 0 ?  "profiling:%s:Overflow writing\n" :
                                  "profiling:%s:Error writing\n",
                                  gf->filename);
      gcov_write_import_file (gf->filename, gi_ptr);
    }
